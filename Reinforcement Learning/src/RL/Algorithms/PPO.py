...
def PPO(env, checkpoint_dir):
    ...
    def learn():
        for _ in range(n_epochs):
            states, actions, probs, values, rewards, dones, batches = agent.replay_buffer.generate_batches()
            ...
            for t in range(len(rewards)-1):
                ...
                for k in range(t, len(rewards) -1):
                    a_t += discount * (rewards[k] + agent.gamma * values[k+1] *(1- int(dones[k])) - values[k])
            ...
            for batch in batches:
                ...
                weighted_clipped_probs = th.clamp(prob_ratio, 1 - agent.policy_clip, 1+agent.policy_clip) * advantage[batch]

...
from src.RL.Networks.ExtractorNetworks import (
    MMExtractor,
...
def save_data(data, path):
    ...
    np.savez(
        os.path.join(path, "states.npz"),
        hand_config=np.array(hand_config),
        hand_torque=np.array(hand_torque),
        finger1=np.array(finger1),
        finger2=np.array(finger2),
        finger3=np.array(finger3),
        palm=np.array(palm),
        obj_count=np.array(obj_count),
        current_obj_locations=np.array(current_obj_locations),
        obj_velocity=np.array(obj_velocity),
        rot_velocity=np.array(rot_velocity),
...
def temp_train():
    ...
    #         tensorboard_log=LOG_DIR,
    #     )
    # model = PPO.load('/media/rpal/Drive_10TB/John/Control Drop/control_dropping/src/RL/Training/Checkpoints/TransformerFeatureEncoder/GPT-EZ_172500_steps.zip')
    # env= BerrettHandGym(
    #                 object_type='Sphere',
    #                 object_quantity=7,
...
#     features_extractor_class=MultiModalTransformerFeatureEncoder,
#     net_arch=[512, 256, 64],
#     #  optimizer_class=th.optim.Adam,
#     #     optimizer_kwargs=dict(
#     #             eps=1e-5,
...
# model = model.learn(1000000,callback=CheckpointCallback(SAVE_FREQ, CHECKPOINT_DIR, 'GPT-EZ'))
